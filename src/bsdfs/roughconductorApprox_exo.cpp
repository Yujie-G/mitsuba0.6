...
public:
	RoughConductorApprox(const Properties& props) : BSDF(props) {
		...
		m_R0_1 = new ConstantSpectrumTexture(
			props.getSpectrum("R0_1", Spectrum(1.0f)));

		m_decay_to = props.getFloat("decay_to", 0.0f); // [fjh] the weight of R0 is 1 at peak. Decide the weight of R0 at edge.
	...
	RoughConductorApprox(Stream* stream, InstanceManager* manager)
		...
		m_R0_1 = static_cast<Texture*>(manager->getInstance(stream));
	...
	void serialize(Stream* stream, InstanceManager* manager) const {
		...
		manager->serialize(stream, m_R0_1.get());
	}
	...
	void configure() {
		...
		if (!m_alphaU->isConstant() || !m_alphaV->isConstant() ||
			...
			!m_R0->isConstant() || !m_R0_1->isConstant() )
		...
		m_usesRayDifferentials =
			...
			m_R0->usesRayDifferentials();
			m_R0_1->usesRayDifferentials();
	...
	template <class T>
	Spectrum fresnel_Schlick(const BSDFSamplingRecord& bRec, T& H) const {

		float temp = 1.0f - dot(bRec.wi, H);
		float temp_5 = pow(temp, 5);

		Spectrum evalR0 = m_R0->eval(bRec.its);
		Spectrum evalR0_1 = m_R0_1->eval(bRec.its);
		float weight = 1.0f - (1.0f - bRec.wo.z) * (1.0f - m_decay_to);
		Spectrum evalRes = evalR0 * weight + evalR0_1 * (1.0f - weight);

		Spectrum result = evalRes + (Spectrum(1.0f) - evalRes) * temp_5;
		return result;
	...
	Spectrum eval(const BSDFSamplingRecord& bRec, EMeasure measure) const {
		...
		/* Fresnel factor */
		const Spectrum F = fresnel_Schlick(bRec, H) *  //fresnelConductorExact(dot(bRec.bRec.wi, H), m_eta, m_k) *
	...
	Spectrum sample(BSDFSamplingRecord& bRec, const Point2& sample) const {
		...
		Spectrum F = fresnel_Schlick(bRec, m) * //fresnelConductorExact(dot(bRec.wi, m), m_eta, m_k) * 
			m_specularReflectance->eval(bRec.its);
	...
	float* bsdfList(int sampleCount, int length, int seed, int highspp=1) const {
		...
		//* [fjh] OMP stuffs *//
		uint tcount = mts_omp_get_max_threads();
...
#pragma omp parallel for
	...
	void addChild(const std::string& name, ConfigurableObject* child) {
		if (child->getClass()->derivesFrom(MTS_CLASS(Texture))) {
			if (name == "alpha")
				...
				m_R0_1 = static_cast<Texture *>(child);
			else
	...
	std::string toString() const {
		...
		oss << "RoughConductorApprox[" << endl
			...
			<< "  R0 = " << indent(m_R0.toString()) << endl
			<< "  R0_1 = " << indent(m_R0_1.toString()) << endl
			<< "  decay_to = " << m_decay_to << endl
			<< "]";
		return oss.str();
	}
...
public:
	RoughConductorApproxShader(Renderer* renderer, const Texture* specularReflectance,
		...
		m_R0_1Shader = renderer->registerShaderForResource(m_R0_1.get());
		// m_R0_1 = _R0_1;////fresnelConductorExact(1.0f, eta, k);
	}
	...
	void putDependencies(std::vector<Shader*>& deps) {
		...
		deps.push_back(m_R0Shader.get());
		deps.push_back(m_R0_1Shader.get());
	}
	...
	void cleanup(Renderer* renderer) {
		...
		renderer->unregisterShaderForResource(m_R0_1.get());
	}
	...
	void bind(GPUProgram* program, const std::vector<int>& parameterIDs, int& textureUnitOffset) const {
		...
		// program->setParameter(parameterIDs[0], m_R0_1);
	}
...
private:
	...
	ref<const Texture> m_R0_1;
	ref<Shader> m_R0Shader;
	ref<Shader> m_R0_1Shader;
};
...
Shader* RoughConductorApprox::createShader(Renderer* renderer) const {
	...
	return new RoughConductorApproxShader(renderer,
		m_specularReflectance.get(), m_alphaU.get(), m_alphaV.get(), m_R0_1.get());
}
