...
public:
	...
	Spectrum fresnel_Schlick(const BSDFSamplingRecord& bRec, Vector& H) const {

		float temp = 1.0f - dot(bRec.wi, H);
		float temp_5 = pow(temp, 5);
		Spectrum result = m_R0->eval(bRec.its) + (Spectrum(1.0f) - m_R0->eval(bRec.its)) * temp_5;
		return result;

	}

	Spectrum fresnel_Schlick(const BSDFSamplingRecord& bRec, Normal& H) const {
