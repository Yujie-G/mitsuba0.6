...
public:
	MultiLayeredBSDF(const Properties &props) : BSDF(props) {
		...
		m_bidir = props.getBoolean("bidir", false);
	...
	void setParameters(const BSDFSamplingRecord &_bRec, std::vector<Frame> &frames,
		ref_vector<Medium> &mediums, int thread_id = -1) const {

		/*
			[fjh] add thread_id to avoid python data generation bug
			use a RUNNING_TID to make sure each thread has its own \code{mediums}
			only take effects at when calling eval() by python with OpenMP
			for cpp rendering or other single-thread usage, thread_id = -1 then everything stays the same
		*/

		Point2 uv = _bRec.its.uv;
		if (thread_id < 0)
			thread_id = Thread::getID() & NUM_COUNTERS_MASK; // NUM_COUNTERS_MASK = 128. For cpp rendering, the thread_id always > 0, but for python it's always 0
		else
			RUNNING_TID++;
	...
	void setParametersPdf(const BSDFSamplingRecord &_bRec,
		...
		Point2 uv = _bRec.its.uv;
		if (thread_id < 0)
			thread_id = Thread::getID() & NUM_COUNTERS_MASK;
		else
			RUNNING_TID++;
	...
	//* [fjh] This is the real "eval()" function called by path_layered.cpp
		...
		if (!(BSDF::getType() & BSDF::ETransmission) && (Frame::cosTheta(bRec.wi) <= 0 || Frame::cosTheta(bRec.wo) <= 0)) {
		...
		else {
			{
				...
				if (m_bidir) {
					...
					SLog(EDebug, "evalVal: %f %f %f", evalVal[0], evalVal[1], evalVal[2]);
				}
			}
			{
				// sample pdf, eval pdf
	...
	Spectrum eval_ProxyFunction(const BSDFSamplingRecord &_bRec, EMeasure measure, int thread_id = -1) const {
		/* 
			[fjh] this is a proxy function for eval (NOT evalAndSample, because we only use eval in bsdfList).
			with this, all usages for eval and evalAndSample with path or path_layered will stay the same
		*/
		Assert(_bRec.sampler);
		...
		std::vector<Frame> frames;
		ref_vector<Medium> mediums;
		setParameters(bRec, frames, mediums, thread_id);
	...
	Spectrum eval(const BSDFSamplingRecord &_bRec, EMeasure measure) const {
		eval_ProxyFunction(_bRec, measure);
	}
...
#pragma omp parallel for
		...
		for (int currentCount = 0; currentCount < sampleCount; currentCount++){
			...
			Vector wi(0.f, 0.707f, 0.707f), wo(0.f, 0.707f, 0.707f);

			//* [fjh] run eval *//
			Intersection its;
			its.uv = currentCount >= sampleCount / 2 ? Point2(0.25f, 0.25f) : Point2(0.75f, 0.75f);
			BSDFSamplingRecord bRec(its, wi, wo, ERadiance);
			bRec.typeMask = BSDF::EAll;
			bRec.sampler = sampler;
			Spectrum value(0.0f);
			for (int i = 0; i < highspp; i++)
				value += eval_ProxyFunction(bRec, ESolidAngle, RUNNING_TID & NUM_COUNTERS_MASK); // [fjh] avoid threading bug. see setParameters().
			value /= highspp * 1.0;
