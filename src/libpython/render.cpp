...
static InternalFloatArray bsdf_bsdfList(BSDF *bsdf, int sampleCount, int length, int seed, int highspp) {
	float *data = bsdf->bsdfList(sampleCount, length, seed, highspp);

	int arraySize = sampleCount * length;
	return InternalFloatArray(bsdf, data, arraySize);
}

static void brdf_deletebsdfList(BSDF *bsdf, InternalFloatArray data)
{
	bsdf->deletebsdfList(data.getPointer());
}
//* [fjh] end bsdfList *//
...
void export_render() {
    ...
    BP_STRUCT(BSDFSamplingRecord, (bp::init<const Intersection &, Sampler *, ETransportMode>()))
		...
		.def_readwrite("sampler", &BSDFSamplingRecord::sampler)
    ...
    BP_CLASS(BSDF, ConfigurableObject, bp::no_init)
        ...
        .def("pdf", &BSDF::pdf)
        .def("bsdfList", bsdf_bsdfList, BP_RETURN_VALUE)
		.def("deletebsdfList", brdf_deletebsdfList)
