...
public:
    ...
    Spectrum Li(const RayDifferential &r, RadianceQueryRecord &rRec) const {
        ...
        if (!its.isValid()) {
            ...
            Li.fromLinearRGB(-1.f, -1.f, -1.f);
            return Li;
        }

        //! [fjh] this outputs the surface nomral (intersection)
        //! [fjh] if a normalmap BSDF is applied, it won't be aware of that. i.e., still output the normal before normalmap perturbation.
        Li.fromLinearRGB(-its.shFrame.n.x * 0.5f + 0.5f, -its.shFrame.n.y * 0.5f + 0.5f, -its.shFrame.n.z); 
        return Li;  
    }
