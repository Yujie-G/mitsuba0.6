//* [fjh] this is a path tracer to render by certain texel of BTF / SVBRDF *//
...
public:
	MIPathTracer(const Properties &props)
		...
		m_uv = Point2(props.getFloat("u", -1.0f), props.getFloat("v", -1.0f));
		assert(m_rrDepth = 2);
	}
	...
	Spectrum Li(const RayDifferential &r, RadianceQueryRecord &rRec) const {
		...
		while (rRec.depth <= m_maxDepth || m_maxDepth < 0) {
			...
			//* [fjh] assign expected uv location
			if (m_uv.x >= 0 && m_uv.y >= 0)
				its.uv = m_uv;
            ...
            if (rRec.type & RadianceQueryRecord::EDirectSurfaceRadiance &&
                ...
                if (!value.isZero()) {
					...
					bRec.sampler = rRec.sampler; // [fjh] for multilayered BSDFs
