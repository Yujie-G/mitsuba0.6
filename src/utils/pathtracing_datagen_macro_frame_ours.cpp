...
public:
	...
	double render(Scene *scene, std::string outPath)
	{
		...
		int bufX = props.getInteger("bufX", 400); 
		int bufY = props.getInteger("bufY", 400);
		int numBuf = props.getInteger("numBuf");
		float rX = props.getFloat("rX", 5.f);
		float rY = props.getFloat("rY", 5.f);
		SLog(EInfo, "wi: %s, wo: %s, u: %f, v: %f, buf: %d x %d", inputWi.toString().c_str(), inputWo.toString().c_str(), inputU, inputV, bufX, bufY);
		...
		for (int curBuf = 0; curBuf < numBuf; curBuf++)
		{
			
			// create a buffer for all the information
			std::vector<float> image(bufX * bufY * NUM_CHANNEL);

			#pragma omp parallel for
			for (int count = 0; count < bufX * bufY; count++)
			{
				...
				float u = 0.0f, v = 0.0f;
				Point2 samplePos(Point2(count % bufY, count / bufY) + Vector2(samplers[tid]->next2D())); // film space
				if (inputU == -2.f)
				{
			...
			snprintf(suffix, buffer_size, "_%d.exr", curBuf);
			writeOutput(image, outPath + std::string(suffix), Vector2i(bufX, bufY));
		...
		while ((optchar = getopt(argc, argv, "D:o:L:p:vqh")) != -1)
		{
			switch (optchar) {
				...
				case 'p':
					...
					if (*end_ptr != '\0')
						SLog(EError, "Could not parse the processor curDir!");
					break;
                case 'q':
                    quietMode = true;
                    break;
