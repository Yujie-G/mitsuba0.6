...
public:
	void help() {
		...
		cout <<  "   -p curDir    Override the detected number of processors. " << endl << endl;
	...
	double render(Scene *scene, std::string outPath)
	{
		...
		const int numQuery = btf_model.btf->LightCount * btf_model.btf->ViewCount;
		...
		for (int curTexel = 0; curTexel < resoX * resoY; curTexel++)
		{
			...
			for (int curDir = 0; curDir < numQuery; curDir++)
			{
				int tid = mts_omp_get_thread_num();

				// [fjh] generate wi wo
				int viewIndex = curDir % btf_model.btf->LightCount;
				int lightIndex = curDir / btf_model.btf->LightCount;
				Vector wi(
				...
				// [fjh] assign values
				auto it = image.begin() + curDir * NUM_CHANNEL;
				*(it++) = bsdfVal[0];
				*(it++) = bsdfVal[1];
				*(it++) = bsdfVal[2];
