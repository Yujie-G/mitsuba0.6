...
if [[ "$(uname)" == 'Darwin' ]]; then
	...
	export PYTHONPATH="$MITSUBA_DIR/dist-debug/python:$MITSUBA_DIR/dist-debug/python/$pyver:$PYTHONPATH"
	mitsuba_plugin_dir="$MITSUBA_DIR/dist-debug/plugins"
fi
unset pyver
...
if [[ "$(uname)" == 'Darwin' ]]; then
	...
	export LD_LIBRARY_PATH="$MITSUBA_DIR/dist-debug:$LD_LIBRARY_PATH"
	export PATH="$MITSUBA_DIR/dist-debug:$PATH"
fi
