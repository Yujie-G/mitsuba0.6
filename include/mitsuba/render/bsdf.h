//* [fjh] add bsdfList for data generation
//* [fjh] added isNeural, isNormalmap, getIndex for python rendering *//
...
public:
    ...
    inline virtual bool isNeural() const {
        return false;
    }

    //* [fjh] for decoder_normalmap *//
    inline virtual bool isNormalMap() const {
        return false;
    }
	...
	/**
	 * \brief [fjh] brdfList, get bsdf values according to a range of wis and wos.  
	 * Returns an InternalFloatArray of size [sampleCount * 7], only contains ONE hemisphere. 
	 * The 7 digits are [wi_x, wi_y, wo_x, wo_y, bsdf_R, bsdf_G, bsdf_B]
	 * \param sampleCount the total number of samples (must be 2^2n)
     * \param length the length of each data entry 
	 * \param seed random seed (Default: -1, need modified $sampler)
	 * \param highspp if given, sample several times to reduce noise (Default: 1)
	 * \return InternalFloatArray (float*) 
	 */
	virtual float* bsdfList(
		int sampleCount, int length, int seed = -1, int highspp = 1
	) const {
        SLog(EError, "[bsdfList] Not Implemented Error.");
		Assert(false);
		return NULL;
	}

	/**
	 * \brief [fjh] deletebsdfList, free the memory
	 * \param list a ptr to brdfList
	 * \return void
	 */
	virtual void deletebsdfList(float* list) const {
        SLog(EError, "[deletebsdfList] Not Implemented Error.");
        Assert(false);
    }
